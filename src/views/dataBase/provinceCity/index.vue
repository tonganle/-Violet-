    ...
    <el-row :gutter="10" class="btn-area">
      <el-col :span="24">
        <!-- <div v-for="(item,index) in grandson" :key='index'>
          <el-button v-if="item.MDL_CODE === 'add'" size="mini" type="primary" plain @click="add">新增省市</el-button>
          <el-button v-if="item.MDL_CODE === 'edit'" size="mini" type="primary" plain @click="edit">编辑省市</el-button>
          <el-button v-if="item.MDL_CODE === 'delete'" size="mini" type="primary" plain @click="del">删除省市</el-button>
        </div>-->
      ...
      >
        <el-row>
          <el-col :span="12">
            <el-form-item
              label="国家名称"
              :prop="messageDisabled == false ? 'PRV_CTY_UUID' : ''"
            >
              <el-select
                v-model.trim="dialogForm.PRV_CTY_UUID"
                filterable
          ...
          <el-col :span="12">
            <el-form-item
              label="省市代码"
              :prop="messageDisabled == false ? 'PRV_CODE' : ''"
            >
              <el-input
                minlength="1"
                maxlength="6"
                v-model.trim="dialogForm.PRV_CODE"
                @change="uppercase('PRV_CODE')"
                :disabled="accountDisabled"
              ></el-input>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item
              label="中文名称"
              :prop="messageDisabled == false ? 'PRV_CHNNAME' : ''"
            >
              <el-input
                minlength="1"
                maxlength="16"
                :disabled="messageDisabled"
                v-model.trim="dialogForm.PRV_CHNNAME"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item
              label="英文名称"
              :prop="messageDisabled == false ? 'PRV_EGSNAME' : ''"
            >
              <el-input
                minlength="1"
                maxlength="16"
                :disabled="messageDisabled"
                v-model="dialogForm.PRV_EGSNAME"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
import { getPCList, getCountryLists, addPC, editPC, delPC } from '@/request/api'
...
export default {
  ...
  methods: {
    ...
    getList(currentPage, pageSize, updateTotal) {
      this.currentData = {}
      getPCList({
        PRV_CHNNAME: this.compForm.cName,
    ...
    getCountryList() {
      getCountryLists({}).then(response => {
        this.countryList = response.data.DXNXTD
      })
    },
    add() {
    ...
    commitEdit(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.isChange.length) {
            this.$set(this.dialogForm, 'PRV_CTY_UUID', this.isChange)
          }
          editPC(this.dialogForm).then(() => {
            this.$message.success({
              message: '编辑数据成功！',
              showClose: true
            })
